...
let getUserFeedbackTemplate = function({rating = 0, comment = '', creationDate, user}, isEditing) {
  ...
  renderFeedbackEntries(element) {
    let renderFeedbackEntries = (element) => {
      ...
      if (this.isFeedbackEnabled()) {
        ...
        } else if (KoliseoAPI.isOAuthConfigured()) {
    ...
    renderFeedbackEntries(element);
    KoliseoAPI.on('login', () => { renderFeedbackEntries(element); }.bind(this));
    KoliseoAPI.on('logout', () => { renderFeedbackEntries(element); }.bind(this));
  }
}
