...
class AgendaView {
  ...
  constructor({
    ...
    if (KoliseoAPI.isOAuthConfigured()) {
      KoliseoAPI.on('login', this.renderUserInfo);
      KoliseoAPI.on('logout', this.renderUserInfo);
  ...
  renderUserInfo() {
    ...
    container.innerHTML = !KoliseoAPI.currentUser?
      `<button onclick="KoliseoAPI.login()" class="ka-button">Sign in</button>` :
      `<button onclick="KoliseoAPI.logout()" class="ka-button ka-button-secondary">Sign out</button>`
    ;
  }
